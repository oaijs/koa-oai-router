...
export default class Responses {
  constructor(schema, validator, errorHandler) {
    this.schema = schema;
  ...
  _defaultErrorHandler_(error, ctx, schema) {
    ...
    if (status === 400)
      respSchema = schema.responses[String(status)] || paramErrorSchema;
    else
      respSchema = schema.responses[String(status)] || schema.responses.default || defaultErrorSchema;
